...
std::tuple<Tensor, Tensor, Tensor> mkldnn_batch_norm_relu(
    ...
    double momentum,
    double eps) {
  TORCH_CHECK(false, "mkldnn_batch_norm_relu: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_batch_norm_relu_backward(
    ...
    double eps,
    std::array<bool,3> grad_input_mask) {
  TORCH_CHECK(false, "mkldnn_batch_norm_relu_backward: ATen not compiled with MKLDNN support");
}
